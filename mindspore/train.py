...
from mindspore import nn, train, Model, context, SummaryCollector, save_checkpoint
...
# forward_fn: 前向传播函数，用于计算网络的输出和损失值
# None: 表示不需要对输入数据进行求导，因为这里只需要计算梯度，而不需要计算输入数据的梯度
# opt.parameters: 优化器的参数，即需要更新的网络参数
...
# 实例化模型
resnet_model = Model(network, loss_fn, opt, metrics={"Accuracy": train.Accuracy()})
...
for epoch in range(num_epochs):
    ...
    # 每个epoch结束后，验证准确率
    acc = resnet_model.eval(dataset_val)['Accuracy']
